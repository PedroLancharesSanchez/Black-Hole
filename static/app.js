...
async function handleLoadImages() {
    ...
    try {
        ...
        updateProgress(5, 'Cargando imágenes...');
        ...
        const response = await fetch('/api/load-images', {
            ...
            body: JSON.stringify({
                ...
                method: elements.reductionMethod.value
            })
        });

        if (!response.ok) {
            const error = await response.json();
            throw new Error(error.error || 'Error al cargar imágenes');
        }

        updateProgress(70, `Aplicando ${elements.reductionMethod.options[elements.reductionMethod.selectedIndex].text}...`);

        const data = await response.json();
        appState.points = data.points;
        appState.connections = data.connections;

        updateProgress(90, 'Renderizando visualización...');
